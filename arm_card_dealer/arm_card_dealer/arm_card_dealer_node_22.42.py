...
class ArmCardDealerNode(Node):
    ...
    def move_to_predefined_position(
        self, position_var, position_name="predefined", tolerance=0.03, velocity=15.0
    ):
        """
        移动到预定义位置
    ...
    def wait_for_arrival(self, target_x, target_y, target_z, tolerance=0.005):
        ...
        self.get_logger().warn(
            f"等待到达 ({target_x:.3f}, {target_y:.3f}, {target_z:.3f}) 超时，当前位置: {current_pos_str}{time_diff_str} 误差: {distance}, 允许误差：{tolerance}"
        )
        return False

    def adjust_end_effector_z_from_predefined(
        self, position_var, z_adjustment, tolerance=0.005, velocity=5.0
    ):
        """
        ...
        # 等待移动完成，检查是否到达目标位置
        return self.wait_for_arrival(x, y, target_z, tolerance)
    ...
    def flip_card(self):
        ...
        # # arm 翻牌吸牌位置
        # if not self.adjust_end_effector_z_from_predefined(self.flip_pickup_pos, 0.01):
        #     self.get_logger().error("未能调整末端执行器Z轴位置")
        #     return False
        if not self.adjust_end_effector_z_from_predefined(self.flip_pickup_pos, 0.05):
            self.get_logger().error("未能调整末端执行器Z轴位置")
            return False

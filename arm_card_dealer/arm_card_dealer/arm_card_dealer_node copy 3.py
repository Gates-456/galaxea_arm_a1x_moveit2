...
class ArmCardDealerNode(Node):
    ...
    def move_to_predefined_position(self, position_var, position_name="predefined"):
        ...
        # 等待到达位置，允许1mm误差
        return self.wait_for_arrival(x, y, z, tolerance=0.005)
    ...
    def wait_for_arrival(self, target_x, target_y, target_z, tolerance=0.01):
        ...
        self.get_logger().warn(
            f"等待到达 ({target_x:.3f}, {target_y:.3f}, {target_z:.3f}) 超时，当前位置: {current_pos_str}{time_diff_str}"
        )
        return False
    ...
    def adjust_end_effector_z_from_predefined(
        ...
        if num_steps == 0:
            self.get_logger().info(f"Z轴调整太小: {z_adjustment}m, 直接设置")
            # 直接移动到目标位置
            adjusted_position = [x, y, target_z, ox, oy, oz, ow]
            return self.move_to_predefined_position(adjusted_position, "final_position")
        ...
        self.get_logger().info(
            f"开始Z轴调整: {z_adjustment}m 分 {num_steps} 步，每步 {actual_step:.4f}m"
        )

        for i in range(1, num_steps + 1):
            # 计算当前位置
            new_z = z + step_multiplier * actual_step * i

            # 确保每一步都不低于安全距离
            if new_z < min_safe_z:
                new_z = min_safe_z
                self.get_logger().warn(
                    f"第 {i} 步Z轴位置低于安全距离，限制为 {min_safe_z}m"
                )

            # 创建当前位置的完整位置列表
            current_position = [x, y, new_z, ox, oy, oz, ow]
            ...
            if not success:
                self.get_logger().error(f"第 {i} 步移动失败")
                return False
            ...
            if i % 10 == 0 or i == num_steps:
                ...
                self.get_logger().info(
                    f"Z轴调整进度: {progress_percent:.1f}% ({i}/{num_steps})"
                )

            # 短暂延时，让移动执行
            time.sleep(0.01)  # 10ms的小延时，让系统有时间处理移动命令

        self.get_logger().info(f"Z轴调整完成: 从基础位置偏移 {z_adjustment}m")
        return True
    ...
    def pickup_card(self, card_surface_height):
        ...
        if not self.adjust_end_effector_z_from_predefined(
            self.pickup_top_pos, card_surface_height, 0.1
        ):

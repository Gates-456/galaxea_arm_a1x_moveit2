...
class ArmCardDealerNode(Node):
    ...
    def move_to_predefined_position(
        self, position_var, position_name="predefined", tolerance=0.01, velocity=52.0
    ):
        """移动到预定义位置"""
    ...
    def flip_card(self):
        ...
        try:
            ...
            # 5. 执行翻牌动作
            slot_result = self.controller_app.slot_machine_control(True)
            if not slot_result:
                self.get_logger().error("翻牌动作失败")
                return False
            ...
            # 11. 调整到吸牌位置
            if not self.adjust_end_effector_z_from_predefined(
                self.flip_pickup_pos, 0.05, tolerance_1=0.01, tolerance=0.01
            ):
                self.get_logger().error("未能调整末端执行器Z轴位置")
                return False
    ...
    def drop_card(self):
        ...
        try:
            ...
            store_up_future = self.thread_pool.submit(
                self.move_to_predefined_position, self.store_up_pos, "store_up_pos"
            )
